...
function HomeCard() {
    ...
    return (
        <Link href={`video/watch?video=${id}`} className='flex group flex-col relative  text-white w-[250px] overflow-hidden sm:w-[380px]'>
            {/* Image || Video */}
            <div className='relative sm:h-[225px] w-full h-[150px] cursor-pointer'>
                <div className='w-full h-full'>
                    <Image loading='lazy' alt='thumbnail' className='rounded-md transition-opacity duration-[2000] ease-in-out group-hover:hidden block w-full h-full' src={image} />
                    <video src='/public/video.mp4' className='group-hover:block transition-opacity duration-[2000] ease-in-out hidden' width="100%" height="100%" muted autoPlay controls />
                </div>
